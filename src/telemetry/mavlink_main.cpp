...
Mavlink::Mavlink(void):
	...
	_notify_pub(IPC_ID(actuator_notify)),
	_param(nullptr)
{
...
void Mavlink::send_bytes(const uint8_t *buf, unsigned packet_len)
{
	...
	ret = hal_uart1_write((void *)buf, packet_len);
...
IPCPull *Mavlink::add_orb_subscription(const ipc_id_t topic)
{
	...
	IPCPull *sub_new = new IPCPull(topic);
...
void Mavlink::run(void *parameter)
{
	...
	configure_stream("SENSOR_IMU",   20.0f);
    ...
    while (1)
    {
		...
		if(_ros_connected != is_connected()) {
			_ros_connected = is_connected();
			...
			if(_ros_connected) {
				led.led_status = LED_PATTERN_BGC_MOD1;
			} else {
				led.led_status = LED_PATTERN_BGC_FAST;
			}
			_notify_pub.push(&led);
		}
